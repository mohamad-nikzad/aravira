...
describe('isTelegramInlineButtonUrl', () => {
  it('accepts https URLs', () => {
    expect(isTelegramInlineButtonUrl('https://app.example/requests?focus=1')).toBe(true)
  })

  it('rejects http and localhost (Telegram inline keyboard rules)', () => {
    expect(isTelegramInlineButtonUrl('http://localhost:3000/requests?focus=1')).toBe(false)
