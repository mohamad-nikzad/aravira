            ...
            <SectionTitle
              ...
              action={
                <Button
                  ...
                  asChild
                >
                  <Link href={`/calendar?date=${date}`}>همه</Link>
                </Button>
              }
            >
              صف امروز
        ...
        <header className="border-b border-line-soft bg-card px-5 pt-3.5 pb-4">
          <div className="flex items-start justify-between gap-3">
            ...
            <Button
              ...
              asChild
            >
              <Link href={`/calendar?date=${todayDate}`} aria-label="تقویم">
          ...
          <Card className="border-line-soft shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between gap-2 pb-2">
              ...
              <Button
                ...
                asChild
              >
                <Link href={`/calendar?date=${tomorrowDate}`}>
                  باز کردن در تقویم
                </Link>
              </Button>
            </CardHeader>
...
export default function TodayPage() {
  ...
  const managerKey =
    user?.role === "manager" ? `/api/today?date=${managerDate}` : null;
  const {
  ...
  const { data: staffDirectoryData, mutate: mutateStaffDirectory } =
    useSWR<StaffResponse>(
      user?.role === "manager" ? "/api/staff" : null,
      fetcher,
    );
  const { data: servicesData, mutate: mutateServices } =
    useSWR<ServicesResponse>(
      user?.role === "manager" ? "/api/services" : null,
      fetcher,
    );
  const { data: clientsData, mutate: mutateClients } = useSWR<ClientsResponse>(
    user?.role === "manager" ? "/api/clients" : null,
    fetcher,
  );
  ...
  const staffTodayKey =
    user?.role === "staff" ? `/api/today?date=${staffTodayDate}` : null;
  const staffTomorrowKey =
    user?.role === "staff" ? `/api/today?date=${staffTomorrowDate}` : null;
  ...
  if (user.role === "manager") {
    ...
    return (
      <ManagerTodayView
        ...
        isLoading={
          (managerLoading || managerIdb.idbLoading) && !managerDisplayData
        }
        error={managerError}
