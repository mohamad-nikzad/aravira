...
function CalendarPageContent() {
  ...
  const { data: staffData, mutate: mutateStaff } = useSWR<StaffResponse>(
    user ? "/api/staff" : null,
    fetcher,
  );
  ...
  const { data: clientsData, mutate: mutateClients } = useSWR<ClientsResponse>(
    user && isManager ? "/api/clients" : null,
    fetcher,
  );
  const { data: businessData, mutate: mutateBusiness } =
  ...
  useEffect(() => {
    ...
    const appointment = appointments.find(
      (item) => item.id === appointmentIdParam,
    );
    ...
    (appointment: AppointmentWithDetails) => {
      ...
      void mutateAppointments(
        (current) => {
          ...
          const withoutAppointment = base.filter(
  ...
  const handleAppointmentClick = (appointment: AppointmentWithDetails) => {
    // Merged/concurrent view only makes sense in time-grid views (day/week),
    // where overlapping appointments visually collide. In list/month each
    // appointment is its own row, so open its detail directly.
  ...
  const handleConcurrentSelect = useCallback(
    (appointment: AppointmentWithDetails) => {
      ...
      requestAnimationFrame(() => setSelectedAppointment(appointment));
    },
    [],
  );
