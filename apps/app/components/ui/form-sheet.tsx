 ...
 *
 * Differences vs the regular Drawer:
 * - Takes the full viewport height (no peek-through to content behind).
 * - `dismissible={false}` blocks swipe-down and outside-tap, so accidental
 *   gestures cannot discard form input. Use an explicit Cancel button.
 * - No drag handle (would invite the same accidental dismiss).
 */
function FormSheet({
  open,
  onOpenChange,
    ...
    <DrawerPrimitive.Portal>
      ...
      >
        <button
          ...
          aria-label="بستن"
          className="absolute top-[calc(0.75rem+env(safe-area-inset-top))] left-3 z-10 flex size-10 items-center justify-center rounded-full bg-secondary/70 text-foreground/75 transition-colors hover:bg-secondary hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring/50 focus-visible:outline-none"
        >
...
function FormSheetHeader({
  ...
  return (
    <div
      ...
      className={cn(
        'flex flex-col gap-0.5 border-b border-line-soft bg-card px-5 pb-4 pe-14 text-right',
